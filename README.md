...
### 4. Dual-Modality Fusion
- When both a thermal image **and** a temperature matrix are provided for a foot, both models run independently and their probabilities are fused:
...
From your mobile app code (React Native / FLIR Lepton SDK):
```javascript
// The FLIR Lepton SDK returns both a rendered image (bytes) and a temperature matrix (float array).
// Base64-encode the image and send everything as a single JSON body.

const body = {
  left_image_b64:    Buffer.from(leftImageBytes).toString('base64'),
  right_image_b64:   Buffer.from(rightImageBytes).toString('base64'),
  left_temperatures: leftTempMatrix,   // 2-D array of floats, e.g. [[30.1, 30.5, ...], ...]
  right_temperatures: rightTempMatrix,
}

const response = await fetch('http://192.168.1.5:8000/predict/patient/mobile', {
    method: 'POST',
...
// result.left_foot.regions              → { MPA, LPA, MCA, LCA } mean °C per region
...
## Example API Response (Mobile / Combined Dual-Foot Diagnosis)

Response from `POST /predict/patient/mobile` or `POST /predict/patient/combined` — each foot result shows the fused probability alongside individual model contributions.
